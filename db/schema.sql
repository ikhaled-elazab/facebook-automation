-- =============================================================================
-- facebook-automation — SQLite schema (Phase 1)
...
--   * Child tables (comments/replies/dm_messages/groups) replace the JSON arrays
--     in accounts.json. ON DELETE CASCADE keeps them consistent with accounts.
--   * State tables (account_state / seen_comments / dm_sent) replace state/*.
...
--   * Pacing / safety-first fields are modeled now (P1) though the governor that
--     enforces them is wired in P5. UI can edit them as soon as P4 lands.
--   * schema_meta.version supports future idempotent migrations.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS settings (
  ...
  -- Global caps; per-account overrides live on the accounts table.
...
-- ── Accounts (replaces accounts.json top-level objects) ──────────────────────
CREATE TABLE IF NOT EXISTS accounts (
  ...
  -- DM-as-page identity (highest ban-risk feature; per-account opt-in)
  ...
  -- ── Per-account pacing overrides (NULL = inherit global) ──
...
-- ── Account content arrays (replace accounts.json comments/replies/dm/groups) ─
...
CREATE INDEX IF NOT EXISTS idx_account_comments_acct ON account_comments(account_id);
...
CREATE INDEX IF NOT EXISTS idx_account_replies_acct ON account_replies(account_id);
...
CREATE INDEX IF NOT EXISTS idx_account_dm_messages_acct ON account_dm_messages(account_id);
...
CREATE INDEX IF NOT EXISTS idx_account_groups_acct ON account_groups(account_id);

-- ── Runtime state (replaces state/*.txt and state/*.json) ───────────────────
-- One row per account: last seen post id + shared post URLs (JSON array text).
...
CREATE INDEX IF NOT EXISTS idx_seen_comments_lookup ON seen_comments(account_id, post_url);
...
CREATE INDEX IF NOT EXISTS idx_dm_sent_acct ON dm_sent(account_id);

-- ── Action log (feeds the safety governor's daily caps + UI recent-events) ────
-- Every like/comment/share/dm gets a row; governor counts today's rows per acct.
...
-- This created_at-only index lets the P5 governor count today's global actions
-- via an index range scan.
...
-- ── Per-account cycle status (P5 — fixes single-row heartbeat last-writer-wins) ─
-- worker_state is a single row; with N accounts each writing per-cycle status to
-- it, one account's 'error' is immediately overwritten by another's 'running'
-- (the 90s liveness ticker also writes 'running' to it). That masks a failing
-- account. This table holds ONE row PER account so each account's last cycle
-- status is independently observable, while worker_state keeps the single global
