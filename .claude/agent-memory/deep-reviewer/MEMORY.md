...
- [v2-migration-review](project_v2_migration_review.md) — v1->v2 multi-branch migration: CONDITIONAL PASS, no CRITICAL, lossless (8 probes); live target is near-empty clean v1 (seen_comments=0/dm_sent=0) so blast radius tiny; HIGH = post-commit fk_check throws on pre-existing orphan after a successful commit.
